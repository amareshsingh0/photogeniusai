"""
PREMIUM v31 test — validates:
  1. GPU1 v31: MediaPipe hand validation, complexity scoring, targeted negatives
  2. GPU2 v3.2: Dynamic hand detailer, conditional eye detailer

Uses a hand-interaction prompt to stress-test the new hand quality pipeline.
"""
...
# Hand-interaction prompt — triggers hand_interaction + fine_detail complexity categories
PROMPT = "a skilled Indian jeweler carefully holding a delicate gold necklace with both hands, intricate kundan work, workshop table with tools, warm cinematic lighting, shallow depth of field, photorealistic portrait"

print(f"[TEST v31] PREMIUM hand-interaction test")
print(f"[TEST v31] Prompt: {PROMPT}")
print(f"[TEST v31] Expected complexity categories: hand_interaction (holding), fine_detail (jewelry/necklace)")
print(f"[TEST v31] Expected candidates: 5 (base 3 + 2 categories)")
print()
...
# Check transfer format
img_bytes = base64.b64decode(image_b64[:100] + "==")
is_jpeg = img_bytes[:2] == b'\xff\xd8'
transfer_fmt = "JPEG" if is_jpeg else "OTHER"
...
print(f"[GPU1] Transfer: {transfer_fmt} ({transfer_size_kb:.0f} KB)")
print(f"[GPU1] Model: {result.get('model', 'N/A')}")
print()

# === GPU2: Post-processing ===
print(f"[GPU2] Calling orchestrator for PREMIUM post-processing...")
print(f"[GPU2] jury_score={jury_score:.3f} -> expected hand_str={'0.35' if jury_score > 0.70 else '0.40' if jury_score > 0.50 else '0.45'}")
gpu2_payload = {
    "action": "post_process",
    "image": image_b64,
    "prompt": PROMPT,
...
try:
    ...
    print(f"[GPU2] Face count: {gpu2_raw.get('face_count', 'N/A')}")
    ...
    if gpu2_raw.get("status") == "success":
        ...
        if final_b64:
            ...
            out_path = "c:/desktop/PhotoGenius AI/test_premium_v31_output.jpg"
...
print()
print(f"[TEST v31] Complete! Total: GPU1={elapsed}s + GPU2={gpu2_time:.0f}s")
print(f"[TEST v31] Check CloudWatch for: [MEDIAPIPE], [COMPLEXITY], [STRUCTURAL], [HAND-DETAIL], [EYE-DETAIL]")
