...
Multi-path face consistency with ensemble verification.
Target: 99%+ face consistency using ArcFace similarity (target >0.85).

Architecture:
- Path 1: InstantID (IP-Adapter + ControlNet) - baseline
- Path 2: FaceAdapter (optional; high-fidelity face)
- Path 3: PhotoMaker (optional; multi-identity)
- Ensemble: Weighted fusion or select best by ArcFace score
"""
...
class GenerationResult:
    """Result of identity-conditioned generation."""
...
class FaceConsistencyScorer:
    """
    Face consistency scoring using InsightFace ArcFace.
    Target: similarity > 0.85 for 99%+ consistency.
    """
    ...
    def score(self, original: Image.Image, generated: Image.Image) -> float:
        """
        Compute ArcFace cosine similarity between original and generated face.
        Returns value in [0, 1]; target > 0.85.
        """
        if not self._ensure_loaded() or self._app is None:
            return 0.0
        try:
            arr_orig = np.array(original.convert("RGB"))
            arr_gen = np.array(generated.convert("RGB"))
            faces_orig = self._app.get(arr_orig)  # type: ignore[reportOptionalMemberAccess]
            faces_gen = self._app.get(arr_gen)  # type: ignore[reportOptionalMemberAccess]
    ...
    def get_embedding(self, image: Image.Image) -> Optional[np.ndarray]:
        """Extract ArcFace embedding for one face image."""
        if not self._ensure_loaded() or self._app is None:
            return None
        try:
            arr = np.array(image.convert("RGB"))
            faces = self._app.get(arr)  # type: ignore[reportOptionalMemberAccess]
            if not faces:
                return None
...
def _compute_face_similarity(
    ...
    """Use InsightFace ArcFace for precise similarity (target: >0.85)."""
    if scorer is None:
        scorer = FaceConsistencyScorer()
    return scorer.score(original, generated)


# Optional path engines (stubs until models are wired)
class InstantIDEngine:
    """InstantID path (IP-Adapter + ControlNet). Loaded in SageMaker container."""
    ...
    def load(self) -> None:
        """Load InstantID + SDXL; called from SageMaker model_fn."""
        ...
        controlnet_path = (
            instantid_path / "ControlNetModel" if instantid_path.exists() else None
        )
        if controlnet_path and controlnet_path.exists():
            cn = ControlNetModel.from_pretrained(
                str(controlnet_path), torch_dtype=dtype
            )
            self._controlnet = cn.to(self.device)  # type: ignore[reportAttributeAccessIssue]
        base_path = Path(f"{self.model_dir}/sdxl-base")
        hf_id = "stabilityai/stable-diffusion-xl-base-1.0"
        repo = (
            str(base_path) if base_path.exists() and any(base_path.iterdir()) else hf_id
        )
        ...
        if self._controlnet is not None:
            ...
            self._pipe = pipe.to(self.device)  # type: ignore[reportAttributeAccessIssue]
        else:
            pipe = StableDiffusionXLPipeline.from_pretrained(repo, **kwargs)
            self._pipe = pipe.to(self.device)  # type: ignore[reportAttributeAccessIssue]
        try:
    ...
    def generate(
        ...
        """Generate one image with InstantID conditioning."""
...
class FaceAdapterEngine:
    """FaceAdapter path (optional). Stub until FaceAdapter-Plus-v1 is integrated."""
    ...
    def load(self) -> None:
        # TODO: HuggingFace FaceAdapter/FaceAdapter-Plus-v1
        pass
...
class PhotoMakerEngine:
    """PhotoMaker path (optional). Stub until TencentARC/PhotoMaker is integrated."""
...
class IdentityEngineV2:
    """
    Multi-path face consistency engine with ensemble verification.
    AWS/SageMaker compatible (no Modal).
    """
    ...
    def load_all(self) -> None:
        """Load all path engines (call from SageMaker model_fn)."""
    ...
    def generate_with_identity(
        ...
        """
        Generate image with face consistency using specified method or ensemble.

        method: "instantid" | "faceadapter" | "photomaker" | "ensemble"
        """
        if method == "ensemble":
        ...
        if img is None:
            return GenerationResult(
                path=method,
                error=f"{method} returned no image (engine not available or failed)",
            )
    ...
    def _ensemble_generate(
        ...
        """
        1. Generate with all available methods (same seed).
        2. Score each result with ArcFace.
        3. Select best by similarity; optional refinement pass can be added later.
        """
        candidates: List[Tuple[str, Image.Image, float]] = []
        path_names = ["instantid", "faceadapter", "photomaker"]
        ...
        if not candidates:
            return GenerationResult(
                error="Ensemble: no path produced an image (InstantID/FaceAdapter/PhotoMaker not loaded or failed)",
            )
        # Select best by similarity
        best_path, best_img, best_sim = max(candidates, key=lambda x: x[2])
        return GenerationResult(
            image=best_img,
            similarity=best_sim,
            path=best_path,
            scores={"arcface": best_sim},
            guaranteed=(best_sim >= 0.85),
        )
