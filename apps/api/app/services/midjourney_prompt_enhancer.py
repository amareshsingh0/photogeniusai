...
def analyze_scene(prompt: str) -> SceneInfo:
    ...
    if not info.is_indoor and not info.is_outdoor:
        if any(
            w in prompt.lower()
            for w in ["sky", "cloud", "sun", "mountain", "beach", "forest", "street", "park"]
        ):
            info.is_outdoor = True
        elif any(w in prompt.lower() for w in ["room", "wall", "inside", "studio", "interior"]):
            info.is_indoor = True
    ...
    if not info.subject_type:
        info.subject_type = (
            "landscape" if (info.is_outdoor and "person" not in prompt.lower()) else "portrait"
        )
...
# Multi-person / group: each figure complete, correct count, spatial & physics logic
...
# Anatomy & head/limb/count negatives — prevent missing head, extra limbs, merged bodies
...
NEGATIVE_BY_MODE: Dict[str, List[str]] = {
    "REALISM": ["cartoon", "anime", "drawing", "painting", "illustration", "3d render", "cgi"],
    ...
    *,
    ...
    for p in parts:
        ...
        if p_lower not in seen:
            seen.add(p_lower)
            out.append(p)
    return ", ".join(out[:100])  # allow more tokens for anatomy negatives
...
def enhance(
    ...
    *,
    ...
    # 2b) Anatomy boosters when scene has person/body — prevent missing hands, phantom objects
    person_body_markers = [
        "person",
        "man",
        "woman",
