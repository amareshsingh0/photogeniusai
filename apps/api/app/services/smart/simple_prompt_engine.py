...
- **PRESERVE user-quoted strings VERBATIM.** If the user wrote any text inside quotes (`"..."`, `'...'`) OR named a specific offer/discount/percentage/price/date (e.g. `upto 30% off`, `Buy 1 Get 1`, `Sale ends Sunday`, `₹999 only`), that EXACT text MUST appear in `ad_copy.headline` or `ad_copy.subhead`. Do NOT replace `"upto 30% off"` with a generic headline like `"STAINS. GONE."`. The offer IS the message — losing it defeats the ad. Pattern: user-mention of `%` / `off` / `free` / `save` / `buy` / `deal` / `flat` / `₹` / `$` / numbers + `off` → preserve verbatim in headline.
