...
- **MULTILINGUAL TEXT — USE NATIVE SCRIPT, NEVER TRANSLITERATE.** GPT Image 2 renders native scripts at 95%+ accuracy when given the actual native characters. ALWAYS write rendered text in the actual native script the language uses — NEVER in romanized transliteration. Roman-letter transliteration of Hindi/Bengali/Tamil/etc tells the image model to render Latin letters (you get gibberish that looks like English), while the actual native characters render correctly with proper conjuncts and diacritics. Supported scripts the model handles well: Devanagari (Hindi, Marathi, Sanskrit, Nepali, Konkani), Bengali (Bangla, Assamese), Tamil, Telugu, Kannada, Malayalam, Gurmukhi (Punjabi), Gujarati, Oriya, Sinhala, Urdu (Nastaliq Perso-Arabic), CJK (Chinese Hanzi, Japanese Kanji/Kana, Korean Hangul), Arabic, Hebrew, Cyrillic, Thai, Greek, and similar. RULE — applies whenever ANY non-English language is in scope: (1) If the user explicitly names a language ("Hindi", "Marathi headline", "in Tamil", "Bengali poster") -> use that script verbatim. (2) If the prompt context implies a regional market (city + local-language brand, festival/cultural event tied to a language, regional cuisine, regional-language slogan in research) -> use the native script of that region. (3) Mixed-script bilingual is the common real-world pattern: native-script primary headline + smaller English supporting line works well — write both in their actual scripts. (4) Surface real slogans from the web-research block in the native script when the brand uses that script in its real marketing. Quote each text string in straight double quotes in the `prompt` field using the actual native characters (not unicode escape sequences, not transliteration).
...
- **BRAND-IDENTITY LAYER IS OPT-IN, NEVER AUTO-FILLED.** The 4 brand-identity fields below are powerful when the user wants them, but inventing placeholders for them clutters the ad and can be misleading. Treat each one as STRICTLY OPT-IN — populate ONLY when the user explicitly provides the value OR explicitly asks for it in the prompt. Otherwise leave the field empty / null / `[]`.
  ...
  • `website_url` — populate ONLY if the user explicitly provides a URL or explicitly asks for one ("include our website"). NEVER invent placeholder URLs like `www.brandname.in`. An invented URL on a real brand ad is misleading.
...
- **ON-IMAGE TEXT — USE RESEARCH + RULES, NOT RANDOM INVENTION.** When the user has explicitly quoted text ("with text 'BEAT FEST 2026'"), use that VERBATIM in `headline` (and `subhead`/`cta` if multiple lines were given). When the user did NOT specify text but the intent IS an ad/poster/wishes/greeting (anything where text is expected by genre): write the headline using (a) the brand's REAL slogan/tagline if the web-research block mentions one, (b) what the brand's actual ad voice would say per the research, or (c) a tight 2-5 word headline that fits the depicted scene. Never invent a CTA / benefit_line / trust_signal that contradicts the brand's actual offer surfaced in research. If the brand has no quotable tagline in research and the user just said "ad", a single headline + brand name is enough — do NOT pad with auto-generated benefit_lines / trust_signals / "Try It Now" CTAs unless the user or research provides them. Empty is better than fabricated. The web-research block is the authoritative source for brand voice, real taglines, and known campaign hooks; use it.
...
class SimplePromptEngine:
    ...
    async def _critique_with_gemini(
        ...
        review_msg = (
            ...
            "  9. headline_typography / subhead_typography / cta_typography: each populated text field needs font + weight + size + color (+ tracking). If the matching text field (headline/subhead/cta) is empty, leave its typography field empty too.\n"
            ...
            "PHASE 4 - COPYWRITING (anchor on web research + user-quoted text, never random invention):\n"
            " 13. headline: 2-5 WORDS MAX. If the user quoted text, use VERBATIM. If not, prefer the brand's REAL slogan/tagline from the web-research block; fall back to a scene-fitting headline. Pass THUMB TEST + 0.3-second test.\n"
            " 14. cta: 2-3 WORDS, action verb. Use the brand's actual CTA from research when available. Skip the CTA entirely if neither user nor research provides one — do not pad with 'Try It Now / Order Now' just to fill space.\n"
            " 15. benefit_lines: 2-3 words each (icon-badge format). Only include items grounded in research or user input. Reject auto-padded generic lists.\n"
