...
_BUCKET_HINTS = {
    ...
    "vector": (
        ...
        "ICON, MONOGRAM, EMBLEM, BADGE, or APP ICON, treat it as professional brand-identity "
        "work — like a senior brand designer would deliver. Apply these CARDINAL RULES:\n"
        "\n"
        "  ── PHASE A: BRAND DISCOVERY (do this BEFORE picking shapes) ──\n"
        "  Infer from the user's words: WHAT industry? WHAT personality (playful, premium, "
        "technical, organic, rebellious, trustworthy)? WHO is the audience? Pick palette + "
        "typography + mark style FROM THIS, not from default 'tech logo = blue + geometric P'.\n"
        "  Industry palette guides (use as starting point, not law):\n"
        "    • SaaS / dev tools / AI → cool neutrals (charcoal + one accent), monospace or "
        "geometric grotesque, abstract symbol\n"
        "    • Creative platform / design tool → vibrant duo (gradient or 2-color contrast), "
        "expressive sans or custom letterform, motion-suggesting mark\n"
        "    • Finance / legal / B2B → navy + gold or deep blue + white, classical sans or serif, "
        "shield/monogram/wordmark\n"
        "    • Wellness / organic → muted earth + sage / terracotta, humanist sans or rounded, "
        "organic flowing mark\n"
        "    • Food / hospitality → warm reds/oranges + cream, friendly slab or script, "
        "illustrative or hand-drawn mark\n"
        "    • Fashion / luxury → black + one accent (gold/champagne) OR pure black, refined "
        "serif or thin sans, monogram or pure wordmark\n"
        "    • Kids / play / community → bright primary palette (3-4 colors), rounded geometric "
        "or hand-drawn, character or playful symbol\n"
        "  If the user's description doesn't fit cleanly, INVENT a concept that does — don't "
        "default to safe tech-startup blue.\n"
        "\n"
        "  ── PHASE B: MARK TYPE DECISION ──\n"
        "  Pick ONE deliberately: wordmark (letters only, custom-feel typography is the mark — "
        "think FedEx, Coca-Cola, Google), lettermark/monogram (1-3 stylized letters — IBM, HBO, "
        "CNN), pictorial symbol (recognizable real-world object — Apple, Twitter bird, Target), "
        "abstract mark (pure geometry that means nothing literal — Nike swoosh, Adidas trefoil, "
        "Mastercard circles), combination (symbol + wordmark stacked or side-by-side — Adidas full, "
        "Burger King, Starbucks). State which type and WHY it fits the brand.\n"
        "\n"
        "  ── PHASE C: CONCEPT-FIRST CONSTRUCTION ──\n"
        "  Before specifying shapes, write ONE SENTENCE describing the CONCEPT/IDEA the mark "
        "communicates. Examples: 'a flowing ribbon that returns to itself, suggesting connection "
        "and continuity', 'an upward-stepping silhouette evoking growth', 'two overlapping forms "
        "that read as both letter and lens'. Without a concept, the mark is decoration. THEN "
        "translate the concept to specific geometry.\n"
        "\n"
        "  ── PHASE D: TECHNICAL CONSTRAINTS ──\n"
        "  • SCALABILITY — readable from 16px favicon to billboard. NO fine details, NO photoreal "
        "shading, NO drop shadows, NO 3D bevels. Flat color is default; max ONE subtle 2-stop "
        "gradient if it serves the concept.\n"
        "  • NEGATIVE SPACE — ≥25% padding on all sides. Mark on solid clean background "
        "(pure white, off-white, or single brand color).\n"
        "  • SINGLE-COLOR TEST — must work in pure solid black-on-white. Color is bonus, not "
        "load-bearing meaning.\n"
        "  • TEXT CORRECTNESS — if wordmark/combination, the EXACT brand name spelled correctly "
        "in QUOTES is non-negotiable. Specify ONE typeface family with personality matching the "
        "brand (geometric sans = modern tech, humanist sans = friendly, serif = heritage/luxury, "
        "slab = editorial/sturdy, script = elegant/personal, mono = technical/dev). Specify "
        "weight (light/regular/medium/bold/black) and tracking (tight/normal/wide). Letterforms "
        "must be CRISP — no pixel-distortion unless the brand IS pixel-art (e.g. retro gaming).\n"
        "  • GEOMETRIC HARMONY — describe construction logic (circles + squares grid, golden "
        "ratio, equilateral triangle, modular system).\n"
        "\n"
        "  ── PHASE E: ANTI-PATTERNS (force into negative_prompt) ──\n"
        "  'photo background, 3D render, drop shadow, bevel, glossy plastic, realistic texture, "
        "wrong letters, misspelled brand name, extra characters, distorted typography, generic "
        "swoosh + globe + gear cliché, AI-default geometric P/A/B in cyan-and-black, pixelated "
        "letterforms unless brand is pixel-themed, busy ornaments, multiple competing focal points, "
        "watermark, sample text, lorem ipsum'.\n"
        "\n"
        "  ── PHASE F: AVOID OBVIOUS DEFAULTS ──\n"
        "  When the brand name suggests a literal interpretation (e.g. 'Pixium' → pixels, "
        "'Sunrise' → sun), DON'T pick the most obvious symbol unless it adds genuine meaning. "
        "Strong logos earn their concept; weak logos illustrate the name. Push for the second "
        "or third interpretation, not the first.\n"
        "\n"
        "Output `prompt` describes the mark's CONSTRUCTION (concept → shapes → proportions → "
        "palette → typeface) — not a scene. `ad_copy` is EMPTY for pure symbols; for wordmarks/"
        "combinations set `ad_copy.headline` to the literal brand text only. The `prompt` should "
        "read like a brand designer's brief to a vector illustrator, with specifics. If the "
        "request is NOT a logo (flat illustration, sticker, infographic, icon set), ignore "
        "Phases A/B/C/F and just specify shapes, palette, geometry, no photo realism."
    ),
