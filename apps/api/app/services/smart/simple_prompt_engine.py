...
_BUCKET_HINTS = {
    ...
    "vector": (
        ...
        "  Pick ONE deliberately, with reason:\n"
        "    • WORDMARK — typography IS the mark (FedEx, Coca-Cola, Google). Best when the "
        "name is short, distinctive, and the typography can carry full personality.\n"
        "    • LETTERMARK / MONOGRAM — 1-3 stylized initials (IBM, HBO, CNN, P&G). Best for "
        "long names or abstract entities.\n"
        "    • PICTORIAL SYMBOL — recognizable object (Apple, Target, Twitter bird). Best when "
        "an object directly evokes the brand promise.\n"
        "    • ABSTRACT MARK — pure geometry meaning nothing literal (Nike swoosh, Adidas "
        "trefoil, Mastercard circles). Best when no literal symbol fits and you want pure "
        "ownership of a shape.\n"
        "    • COMBINATION — symbol + wordmark (Burger King, Starbucks, Adidas full). Most "
        "versatile; works when symbol can also stand alone.\n"
        "    • EMBLEM / BADGE / STAMP — symbol fully integrated into a circular/shield "
        "container (Harley, Starbucks original, NFL teams). Signals heritage, authenticity, "
        "official credibility — strong 2026 trend against synthetic AI sameness.\n"
        "\n"
        ...
        "  When the brand name suggests a literal interpretation (Pixium → pixels, Sunrise → "
        "sun, Forge → hammer/anvil, Loop → circle), the LAZY designer picks the obvious "
        "symbol. The PROFESSIONAL pushes to the second or third interpretation OR uses the "
        "obvious element ONLY as a subtle micro-anchor inside a more distinctive concept. "
        "Strong 2026 logos earn their meaning through unexpected association, not literal "
        "illustration of the name.\n"
        "\n"
        ...
        "    (B) BRAND IDENTITY BOARD — logo + tagline + feature/service showcase + "
        "(optionally) sub-icons in a presentation layout. Use when user describes a STARTUP / "
        "PLATFORM / SERVICE and lists features ('a platform that does X, Y, Z'), or asks for "
        "a 'brand intro', 'launch graphic', 'identity sheet'. This is what ChatGPT/Midjourney "
        "produce by default for SaaS/product brand prompts and what users actually want when "
        "describing a product. Layout: hero logo top/center → tagline below → horizontal row "
        "of 4-7 small feature icons with single-word labels at bottom. Each feature gets its "
        "own color matching the brand gradient/palette so the system feels cohesive. Use "
        "`ad_copy` schema generously: `brand_name` = the literal brand, `emotional_tagline` = "
        "3-5-word brand promise (Imagine. Design. Inspire. Anywhere.), `benefit_lines` = the "
        "feature list as short labels (Typography, Poster, Ad, Thumbnail, Campaign, etc).\n"
