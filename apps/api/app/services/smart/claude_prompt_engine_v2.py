"""
Claude Prompt Engine v2 — Bucket-aware + Model-specific + Validator + Critic

Architecture:
  DEFAULT PATH  (all buckets, all tiers):
    Stage A: Claude Sonnet 4.6 bucket-specific → Creative Brief JSON
    Stage B: Claude Haiku 4.5 model-specific  → generation params JSON
    Validator: schema + budget + bucket-rule check

  HARD BUCKET PATH (anime, typography, editing, architecture — premium/ultra only):
    Stage A: bucket-specific brief (Sonnet 4.6)
    Stage B: model-specific params (Haiku 4.5)
...
def _extract_json(text: str) -> Dict:
    ...
    if match:
        ...
        try:
            ...
            pass
    logger.warning("[claude-engine] _extract_json failed: %r", text[:200])
    return {}
...
class ClaudePromptEngine:
    ...
    Falls back to heuristic if ANTHROPIC_API_KEY missing or any call fails.
    """
    ...
    def enabled(self) -> bool:
        ...
        return _is_claude_enabled()

    def _get_client(self):
        """Return module-level singleton Anthropic client (created once, reused)."""
        global _claude_client
        if "_claude_client" not in globals() or _claude_client is None:
            import anthropic
            key = os.getenv("ANTHROPIC_API_KEY", "").strip()
            globals()["_claude_client"] = anthropic.Anthropic(api_key=key)
        return globals()["_claude_client"]
    ...
    async def _call(
        ...
        """Single async Claude call — returns raw text."""
        client = self._get_client()
        max_tokens = _STAGE_MAX_TOKENS.get(stage, 800)

        # Use extended thinking for Stage A (Brief) - more complex creative work
        use_thinking = (stage == "brief")
        ...
        if use_thinking:
            ...
            # Standard mode for params/critic (Stage B/C)
            resp = client.messages.create(
                model=self._model,
                max_tokens=max_tokens,
                system=system,
        ...
        # Extract text from response (skip thinking blocks, get only text)
        text_content = ""
    ...
    async def create_brief(
        ...
        try:
            ...
            if issues:
                ...
                if missing_critical:
                    logger.warning("[claude-engine] critical brief fields missing, falling back")
                    return self._heuristic_brief(raw_prompt)
            ...
            logger.info("[claude-engine] brief OK bucket=%s: %s",
                        bucket, brief.get("visual_concept", "")[:60])
            return brief

        except Exception as e:
            logger.warning("[claude-engine] brief failed (%s), heuristic fallback", e)
            return self._heuristic_brief(raw_prompt)
    ...
    async def build_params(
        ...
        if capability_bucket == "typography":
            ...
            if img_prompt and isinstance(img_params, dict):
                ...
                logger.info("[claude-engine] using DesignAgentChain image prompt for typography")
                return result
        ...
        try:
            ...
            logger.info(
                "[claude-engine] CDI OK model=%s→%s prompt=%d chars",
                model_name, recommended_model, len(prompt),
            )
            return result

        except Exception as e:
            logger.warning("[claude-engine] CDI failed (%s), heuristic fallback", e)
            return self._heuristic_params(brief, capability_bucket)
    ...
    async def run_critic(
        ...
        try:
            ...
            if not issues and not additions:
                logger.info("[claude-engine] critic: no issues found for %s", bucket)
                return None
            ...
            logger.info("[claude-engine] critic %s: %d issues", bucket, len(issues))
            return notes.strip() or None

        except Exception as e:
            logger.warning("[claude-engine] critic failed (%s), skipping", e)
            return None
    ...
    async def enhance(
        ...
        use_critic = (
            ...
            and self.enabled
            and brief.get("_source") == "claude"
            and params.get("_source") == "claude"
        )
        if use_critic:
        ...
        engine_label = "claude" if params.get("_source") == "claude" else "heuristic"
